...
func NewTagService(db *gorm.DB, store store.Factory) *tagService {
	return &tagService{
		store: store,
		db:    db,
	}
...
func (s *tagService) Create(c *gin.Context, args *v1.ImageCreateArgs) (*v1.ImageCreateReply, error) {
...
func (s *tagService) Delete(c *gin.Context, args *v1.ImageDeleteArgs) error {
	// 删除镜像前需要做个判断，镜像是否被引用
...
func (s *tagService) GetList(c *gin.Context, args *v1.ImageListArgs) (*v1.ImageListReply, error) {
...
func (s *tagService) Update(c *gin.Context, args *v1.ImageUpdateArgs) error {
...
func (s *tagService) Reload(c *gin.Context, args *v1.ImageReloadArgs) error {
	_, err := s.store.Image().GetBy(c, s.db, s.store.WithID(args.ID))
	if err != nil {
...
func (s *tagService) listQuery(args queryArgs) []store.DBOptions {
