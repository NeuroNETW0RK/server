...
func (s *service) Create(c *gin.Context, args *v1.ImageCreateArgs) (*v1.ImageCreateReply, error) {
...
func (s *service) Delete(c *gin.Context, args *v1.ImageDeleteArgs) error {
	// 删除镜像前需要做个判断，镜像是否被引用
...
func (s *service) GetList(c *gin.Context, args *v1.ImageListArgs) (*v1.ImageListReply, error) {
	...
	imageBos, err := s.store.Image().GetListBy(c, s.db, listQuery...)
	if err != nil {
		log.C(c).Warnf("get image list error: %v", err)
		return nil, err
	}

	for _, bo := range imageBos {
...
func (s *service) Update(c *gin.Context, args *v1.ImageUpdateArgs) error {
...
func (s *service) Reload(c *gin.Context, args *v1.ImageReloadArgs) error {
	_, err := s.store.Image().GetBy(c, s.db, s.store.WithID(args.ID))
	if err != nil {
