...
type ITaskResource interface {
	...
	GetBy(c context.Context, db *gorm.DB, opts ...DBOptions) (taskResource *model.TaskResourceBo, err error)
	GetListBy(c context.Context, db *gorm.DB, opts ...DBOptions) (taskResources []model.TaskResourceBo, err error)
